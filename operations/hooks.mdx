...
sidebarTitle: "Hooks"
description: "Bunker OS configures 7 OpenCode hooks across 4 events — SessionStart, PostCompact, PostToolUse, and Stop — to automate context loading, git commits, and handover generation."
---
...
Context compaction is Claude's mechanism for summarizing old conversation turns when the context window fills up. The problem: hook-injected context from `SessionStart` does not survive compaction. The `PostCompact` hooks re-inject the hot cache immediately after compaction.
